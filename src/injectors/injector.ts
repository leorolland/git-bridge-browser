...
export interface ButtonInjector {
     ...
     * @param currentUrl The currently configured GitBridge URL
     */
...
function openInGitBridge(e: MouseEvent | KeyboardEvent, inNewTab: boolean) {
    const currentUrl = window.location.href;
    window.open(`https://gitbridge.io/#${currentUrl}`, inNewTab ? '_blank' : '_self');
    e.preventDefault();
    e.stopPropagation();
}
    ...
    if (config.rewritePeriodKeybind) {
        document.querySelectorAll('.js-github-dev-shortcut, .js-github-dev-new-tab-shortcut').forEach((elem) => {
            ...
            new_element.addEventListener('click', (e) => {
                if (new_element && isVisible(new_element) && !confirm('Are you sure you want to open gitbridge.io?')) {
                    return;
                }
                openInGitBridge(e, elem.classList.contains('js-github-dev-new-tab-shortcut') || config.openAsPopup);
            });
        });
    }
}
...
export async function rewritePeriodKeybindGitLab() {
    ...
    if (config.rewritePeriodKeybind) {
        ...
        document.onkeydown = (e: KeyboardEvent) => {
            if (e.code === 'Period') {
                openInGitBridge(e, e.shiftKey || config.openAsPopup);
            }
        };
    }
}
